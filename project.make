; Basic Drupal core by Mutasem Elayyoub Whywebs.com
core = 7.x
api = 2

; Core
projects[drupal][type] = "core"

; Modules
; --------
projects[admin_menu][type] = "module"
projects[admin_menu][subdir] = "contrib"

projects[admin_views][type] = "module"
projects[admin_views][subdir] = "contrib"

projects[anonymous_login][type] = "module"
projects[anonymous_login][subdir] = "contrib"

projects[autoload][type] = "module"
projects[autoload][subdir] = "contrib"

projects[addressfield][type] = "module"
projects[addressfield][subdir] = "contrib"

projects[blockify][type] = "module"
projects[blockify][subdir] = "contrib"

projects[block_class][type] = "module"
projects[block_class][subdir] = "contrib"

projects[context][type] = "module"
projects[context][subdir] = "contrib"

projects[inline_conditions][type] = "module"
projects[inline_conditions][subdir] = "contrib"

projects[inline_entity_form][type] = "module"
projects[inline_entity_form][subdir] = "contrib"

projects[ctools][type] = "module"
projects[ctools][subdir] = "contrib"

projects[chosen][type] = "module"
projects[chosen][subdir] = "contrib"

projects[custom_formatters][type] = "module"
projects[custom_formatters][subdir] = "contrib"

projects[content_access][type] = "module"
projects[content_access][subdir] = "contrib"

projects[edit_profile][type] = "module"
projects[edit_profile][subdir] = "contrib"

projects[i18n][type] = "module"
projects[i18n][subdir] = "contrib"

projects[profile2][type] = "module"
projects[profile2][subdir] = "contrib"

projects[autoupload][type] = "module"
projects[autoupload][subdir] = "contrib"

projects[email_registration][type] = "module"
projects[email_registration][subdir] = "contrib"

projects[entitycache][type] = "module"
projects[entitycache][subdir] = "contrib"

projects[entity_embed][type] = "module"
projects[entity_embed][subdir] = "contrib"

projects[entityreference_prepopulate][type] = "module"
projects[entityreference_prepopulate][subdir] = "contrib"

projects[flag_abuse][type] = "module"
projects[flag_abuse][subdir] = "contrib"

projects[hide_block_by_role][type] = "module"
projects[hide_block_by_role][subdir] = "contrib"

projects[gd_infinite_scroll][type] = "module"
projects[gd_infinite_scroll][subdir] = "contrib"

projects[legal][type] = "module"
projects[legal][subdir] = "contrib"

projects[login_tracker][type] = "module"
projects[login_tracker][subdir] = "contrib"

projects[menu_attributes][type] = "module"
projects[menu_attributes][subdir] = "contrib"

projects[menu_html][type] = "module"
projects[menu_html][subdir] = "contrib"

projects[menu_token][type] = "module"
projects[menu_token][subdir] = "contrib"

projects[message][type] = "module"
projects[message][subdir] = "contrib"

projects[roles_for_menu][type] = "module"
projects[roles_for_menu][subdir] = "contrib"

projects[og][type] = "module"
projects[og][subdir] = "contrib"

projects[r4032login][type] = "module"
projects[r4032login][subdir] = "contrib"

projects[realname][type] = "module"
projects[realname][subdir] = "contrib"

projects[redis][type] = "module"
projects[redis][subdir] = "contrib"

projects[search_api_db][type] = "module"
projects[search_api_db][subdir] = "contrib"

projects[timeago][type] = "module"
projects[timeago][subdir] = "contrib"

projects[title][type] = "module"
projects[title][subdir] = "contrib"

projects[ffc][type] = "module"
projects[ffc][subdir] = "contrib"

projects[field_formatter_settings][type] = "module"
projects[field_formatter_settings][subdir] = "contrib"

projects[flashgame][type] = "module"
projects[flashgame][subdir] = "contrib"

projects[field_formatter_class][type] = "module"
projects[field_formatter_class][subdir] = "contrib"

projects[field_group][type] = "module"
projects[field_group][subdir] = "contrib"

projects[eck][type] = "module"
projects[eck][subdir] = "contrib"

projects[date][type] = "module"
projects[date][subdir] = "contrib"

projects[devel][type] = "module"
projects[devel][subdir] = "contrib"

projects[ds][type] = "module"
projects[ds][subdir] = "contrib"

projects[dc_co_pages][type] = "module"
projects[dc_co_pages][subdir] = "contrib"

projects[ds_extra_layouts][type] = "module"
projects[ds_extra_layouts][subdir] = "contrib"

projects[diff][type] = "module"
projects[diff][subdir] = "contrib"

projects[entity][type] = "module"
projects[entity][subdir] = "contrib"

projects[elysia_cron][type] = "module"
projects[elysia_cron][subdir] = "contrib"

projects[entityreference][type] = "module"
projects[entityreference][subdir] = "contrib"

projects[facetapi][type] = "module"
projects[facetapi][subdir] = "contrib"

projects[conditional_fields][type] = "module"
projects[conditional_fields][subdir] = "contrib"

projects[features][type] = "module"
projects[features][subdir] = "contrib"

projects[features_extra][type] = "module"
projects[features_extra][subdir] = "contrib"

projects[feeds][type] = "module"
projects[feeds][subdir] = "contrib"

projects[feed_import][type] = "module"
projects[feed_import][subdir] = "contrib"

projects[feeds_imagegrabber][type] = "module"
projects[feeds_imagegrabber][subdir] = "contrib"

projects[feeds_tamper][type] = "module"
projects[feeds_tamper][subdir] = "contrib"

projects[uuid][type] = "module"
projects[uuid][subdir] = "contrib"

projects[node_export][type] = "module"
projects[node_export][subdir] = "contrib"

projects[print][type] = "module"
projects[print][subdir] = "contrib"

projects[print_pdf_docraptor][type] = "module"
projects[print_pdf_docraptor][subdir] = "contrib"

projects[views_pdf][type] = "module"
projects[views_pdf][subdir] = "contrib"

projects[webform2pdf][type] = "module"
projects[webform2pdf][subdir] = "contrib"

projects[field_collection][type] = "module"
projects[field_collection][subdir] = "contrib"

projects[commerce][type] = "module"
projects[commerce][subdir] = "contrib"

projects[commerce_addressbook][type] = "module"
projects[commerce_addressbook][subdir] = "contrib"

projects[commerce_backoffice][type] = "module"
projects[commerce_backoffice][subdir] = "contrib"

projects[commerce_bank_transfer][type] = "module"
projects[commerce_bank_transfer][subdir] = "contrib"

projects[commerce_billy][type] = "module"
projects[commerce_billy][subdir] = "contrib"

projects[commerce_billy_mail][type] = "module"
projects[commerce_billy_mail][subdir] = "contrib"

projects[commerce_cod][type] = "module"
projects[commerce_cod][subdir] = "contrib"

projects[commerce_coupon][type] = "module"
projects[commerce_coupon][subdir] = "contrib"

projects[commerce_discount][type] = "module"
projects[commerce_discount][subdir] = "contrib"

projects[commerce_features][type] = "module"
projects[commerce_features][subdir] = "contrib"

projects[commerce_paypal][type] = "module"
projects[commerce_paypal][subdir] = "contrib"

projects[commerce_multicurrency][type] = "module"
projects[commerce_multicurrency][subdir] = "contrib"

projects[commerce_fieldgroup_panes][type] = "module"
projects[commerce_fieldgroup_panes][subdir] = "contrib"

projects[conditional_fields][type] = "module"
projects[conditional_fields][subdir] = "contrib"

projects[commerce_product_attributes][type] = "module"
projects[commerce_product_attributes][subdir] = "contrib"

projects[commerce_product_bundle][type] = "module"
projects[commerce_product_bundle][subdir] = "contrib"

projects[commerce_rules_extra][type] = "module"
projects[commerce_rules_extra][subdir] = "contrib"

projects[commerce_rules_pane][type] = "module"
projects[commerce_rules_pane][subdir] = "contrib"

projects[commerce_agree_terms][type] = "module"
projects[commerce_agree_terms][subdir] = "contrib"

projects[file_entity][type] = "module"
projects[file_entity][subdir] = "contrib"

projects[flag][type] = "module"
projects[flag][subdir] = "contrib"

projects[globalredirect][type] = "module"
projects[globalredirect][subdir] = "contrib"

projects[google_analytics][type] = "module"
projects[google_analytics][subdir] = "contrib"

projects[htmlmail][type] = "module"
projects[htmlmail][subdir] = "contrib"

projects[inline_entity_form][type] = "module"
projects[inline_entity_form][subdir] = "contrib"

projects[job_scheduler][type] = "module"
projects[job_scheduler][subdir] = "contrib"

projects[jquery_update][type] = "module"
projects[jquery_update][subdir] = "contrib"

projects[link][type] = "module"
projects[link][subdir] = "contrib"

projects[libraries][type] = "module"
projects[libraries][subdir] = "contrib"

projects[logintoboggan][type] = "module"
projects[logintoboggan][subdir] = "contrib"

projects[imce_private_files][type] = "module"
projects[imce_private_files][subdir] = "contrib"

projects[mailsystem][type] = "module"
projects[mailsystem][subdir] = "contrib"

projects[media][type] = "module"
projects[media][subdir] = "contrib"

projects[migrate][type] = "module"
projects[migrate][subdir] = "contrib"

projects[memcache][type] = "module"
projects[memcache][subdir] = "contrib"

projects[mimemail][type] = "module"
projects[mimemail][subdir] = "contrib"

projects[module_filter][type] = "module"
projects[module_filter][subdir] = "contrib"

projects[name][type] = "module"
projects[name][subdir] = "contrib"

projects[pathauto][type] = "module"
projects[pathauto][subdir] = "contrib"

projects[references][type] = "module"
projects[references][subdir] = "contrib"

projects[rules][type] = "module"
projects[rules][subdir] = "contrib"

projects[registration][type] = "module"
projects[registration][subdir] = "contrib"

projects[save_edit][type] = "module"
projects[save_edit][subdir] = "contrib"

projects[scheduler][type] = "module"
projects[scheduler][subdir] = "contrib"

projects[scheduler_workbench][type] = "module"
projects[scheduler_workbench][subdir] = "contrib"

projects[search_api][type] = "module"
projects[search_api][subdir] = "contrib"

projects[strongarm][type] = "module"
projects[strongarm][subdir] = "contrib"

projects[swiftmailer][type] = "module"
projects[swiftmailer][subdir] = "contrib"

projects[swfembed][type] = "module"
projects[swfembed][subdir] = "contrib"

projects[step][type] = "module"
projects[step][subdir] = "contrib"

projects[token][type] = "module"
projects[token][subdir] = "contrib"

projects[token_filter][type] = "module"
projects[token_filter][subdir] = "contrib"

projects[views][type] = "module"
projects[views][subdir] = "contrib"

projects[views_bulk_operations][subdir] = "contrib"

projects[views_show_more][type] = "module"
projects[views_show_more][subdir] = "contrib"

projects[views_litepager][type] = "module"
projects[views_litepager][subdir] = "contrib"

projects[webform][type] = "module"
projects[webform][subdir] = "contrib"

projects[workbench_moderation][type] = "module"
projects[workbench_moderation][subdir] = "contrib"

projects[wysiwyg][type] = "module"
projects[wysiwyg][subdir] = "contrib"

projects[youtube_import][type] = "module"
projects[youtube_import][subdir] = "contrib"

projects[youtube][type] = "module"
projects[youtube][subdir] = "contrib"

projects[html_import][type] = "module"
projects[html_import][subdir] = "contrib"

projects[smart_import][type] = "module"
projects[smart_import][subdir] = "contrib"




; Auth modules
;-------------------

projects[oauth2_server][type] = "module"
projects[oauth2_server][subdir] = "contrib"

projects[oauth2_login][type] = "module"
projects[oauth2_login][subdir] = "contrib"

projects[services][type] = "module"
projects[services][subdir] = "contrib"

projects[signup][type] = "module"
projects[signup][subdir] = "contrib"

projects[fboauth_profile2][type] = "module"
projects[fboauth_profile2][subdir] = "contrib"

projects[fboauth][type] = "module"
projects[fboauth][subdir] = "contrib"

projects[oauth][type] = "module"
projects[oauth][subdir] = "contrib"

projects[oauth_api][type] = "module"
projects[oauth_api][subdir] = "contrib"

projects[complete_profile][type] = "module"
projects[complete_profile][subdir] = "contrib"

projects[facebook_rules][type] = "module"
projects[facebook_rules][subdir] = "contrib"

projects[pdm][type] = "module"
projects[pdm][subdir] = "contrib"

projects[pdm][type] = "module"
projects[pdm][subdir] = "contrib"





;just for extra modules


;projects[][type] = "module"
;projects[][subdir] = "contrib"


; Ckeditors only
; ---------
projects[ckeditor][type] = "module"
projects[ckeditor][subdir] = "contrib"

projects[ckeditor_swf][type] = "module"
projects[ckeditor_swf][subdir] = "contrib"

projects[imce][type] = "module"
projects[imce][subdir] = "contrib"

projects[media_ckeditor][type] = "module"
projects[media_ckeditor][subdir] = "contrib"

projects[ckeditor_responsive_plugin][type] = "module"
projects[ckeditor_responsive_plugin][subdir] = "contrib"

projects[wysiwyg_ckeditor_dragresize][type] = "module"
projects[wysiwyg_ckeditor_dragresize][subdir] = "contrib"




; Git modules
; --------
projects[entity_browser][type] = "module"
projects[entity_browser][subdir] = "entity_browser"
projects[entity_browser][download][type] = "git"
projects[entity_browser][download][url] = "https://git.drupal.org/project/entity_browser.git"
projects[entity_browser][download][branch] = "7.x-1.x"

projects[apachesolr_file][type] = "module"
projects[apachesolr_file][subdir] = "apachesolr_file"
projects[apachesolr_file][download][type] = "git"
projects[apachesolr_file][download][url] = "https://git.drupal.org/project/apachesolr_file.git"
projects[apachesolr_file][download][branch] = "master"



; Solr modules and search api
;-----------

projects[apachesolr][type] = "module"
projects[apachesolr][subdir] = "contrib"

projects[apachesolr_multisitesearch][type] = "module"
projects[apachesolr_multisitesearch][subdir] = "contrib"




; Graph
; ---------


projects[charts_graphs_flot][type] = "module"
projects[charts_graphs_flot][subdir] = "contrib"

projects[charts_graphs][type] = "module"
projects[charts_graphs][subdir] = "contrib"

projects[flot][type] = "module"
projects[flot][subdir] = "contrib"





; Libraries
; ---------


libraries[flot][download][type] = "git"
libraries[flot][download][branch] = "master"
libraries[flot][download][url] = "https://github.com/flot/flot.git"
libraries[flot][directory_name] = "flot"
libraries[flot][destination] = "libraries"

libraries[d3][download][type] = get
libraries[d3][download][url] = https://github.com/d3/d3/zipball/master
libraries[d3][directory_name] = "d3"
libraries[d3][destination] = "libraries"

libraries[ckeditor][download][type] = get
libraries[ckeditor][download][url] = http://download.cksource.com/CKEditor/CKEditor/CKEditor%204.4.7/ckeditor_4.4.7_standard.zip
libraries[ckeditor][directory_name] = "ckeditor"
libraries[ckeditor][destination] = "libraries"

libraries[colorbox][download][type] = get
libraries[colorbox][download][url] = https://github.com/jackmoore/colorbox/archive/1.6.3.tar.gz
libraries[colorbox][directory_name] = "colorbox"
libraries[colorbox][destination] = "libraries"

libraries[chosen][download][type] = get
libraries[chosen][download][url] = https://github.com/harvesthq/chosen/releases/download/v1.2.0/chosen_v1.2.0.zip
libraries[chosen][directory_name] = "chosen"
libraries[chosen][destination] = "libraries"

libraries[pecl][download][type] = get
libraries[pecl][download][url] = http://git.php.net/?p=pecl/web_services/oauth.git;a=snapshot;h=017b1a39b6dd4eb904d0aa32793c9f6a34a92b4e;sf=zip
libraries[pecl][directory_name] = "pecl"
libraries[pecl][destination] = "libraries"

libraries[zendframework][download][type] = "git"
libraries[zendframework][download][branch] = "master"
libraries[zendframework][download][url] = https://github.com/zendframework/zendframework.git
libraries[zendframework][directory_name] = "zendframework"
libraries[zendframework][destination] = "libraries"

libraries[print_pdf_docraptor][download][type] = get
libraries[print_pdf_docraptor][download][url] = https://github.com/apmsooner/docraptor/archive/master.zip
libraries[print_pdf_docraptor][directory_name] = "print_pdf_docraptor"
libraries[print_pdf_docraptor][destination] = "libraries"

libraries[TCPDF][download][type] = "git"
libraries[TCPDF][download][branch] = "master"
libraries[TCPDF][download][url] = "https://github.com/tecnickcom/TCPDF.git"
libraries[TCPDF][directory_name] = "tcpdf"
libraries[TCPDF][destination] = "libraries"

libraries[FPDF][download][type] = "git"
libraries[FPDF][download][branch] = "master"
libraries[FPDF][download][url] = "https://github.com/Setasign/FPDF.git"
libraries[FPDF][directory_name] = "FPDF"
libraries[FPDF][destination] = "libraries"

libraries[epub][download][type] = "git"
libraries[epub][download][branch] = "master"
libraries[epub][download][url] = "https://github.com/futurepress/epub.js.git"
libraries[epub][directory_name] = "epub"
libraries[epub][destination] = "libraries"


libraries[fancybox][download][type] = get
libraries[fancybox][download][url] = "https://github.com/fancyapps/fancyBox/zipball/v2.1.5"
libraries[fancybox][destination] = libraries
libraries[fancybox][directory_name] = fancybox

libraries[swiftmailer][download][type] = get
libraries[swiftmailer][download][url] = https://github.com/swiftmailer/swiftmailer/archive/v5.4.1.zip
libraries[swiftmailer][directory_name] = "swiftmailer"
libraries[swiftmailer][destination] = "libraries"

libraries[wordcount][download][type] = get
libraries[wordcount][download][url] = http://download.ckeditor.com/wordcount/releases/wordcount_1.10.zip
libraries[wordcount][directory_name] = "wordcount_tmp"
libraries[wordcount][destination] = "libraries/ckeditor/plugins"

libraries[swfobject][download][type] = get
libraries[swfobject][download][url] = https://github.com/swfobject/swfobject/archive/master.zip
libraries[swfobject][directory_name] = "swfobject"
libraries[swfobject][destination] = "libraries"

libraries[ck-dragresize][download][type] = get
libraries[ck-dragresize][download][url] = https://github.com/sstur/ck-dragresize/archive/master.zip
libraries[ck-dragresize][directory_name] = "ck-dragresize"
libraries[ck-dragresize][destination] = "libraries"

; Themes
; --------
projects[zen][type] = "theme"
projects[zen][subdir] = "contrib"

; Themes
; --------
projects[zen][subdir] = "contrib"
